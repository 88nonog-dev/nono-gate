import fs from 'fs';
import path from 'path';
import crypto from 'crypto';

import { execFileSync } from 'child_process';

function computeEngineFingerprint(){
  try{
    const out = execFileSync(process.execPath, ['scripts/engine-fingerprint.js'], { encoding:'utf8' });
    const m = out.match(/ENGINE_FINGERPRINT=([0-9a-f]{64})/i);
    return m ? m[1].toLowerCase() : '';
  }catch{
    return '';
  }
}
import { normalizeSarif } from './normalize.js';
import { diffIssues } from './diff.js';
import { loadPolicy, decide } from './policy.js';
import { writeEvidence } from './evidence.js';
import { finalizeWithLedger } from './ledger/finalize.js';
import { ensureContract, hashContract } from './contract.js';
import { sha256Text } from './hash.js';
import { appendLineageEntry } from './lineage.js';

function readText(p){
  return fs.readFileSync(p,'utf-8');
}

function readJson(p){
  const raw = readText(p);
  const clean = raw && raw.length && raw.charCodeAt(0) === 0xFEFF ? raw.slice(1) : raw; // strip UTF-8 BOM if present
  return clean ? JSON.parse(clean) : null;
}

function canonicalize(v){
  if(v === null) return null;
  const t = typeof v;
  if(t === 'number' || t === 'boolean' || t === 'string') return v;
  if(Array.isArray(v)) return v.map(canonicalize);
  if(t === 'object'){
    const out = {};
    for(const k of Object.keys(v).sort()){
      out[k] = canonicalize(v[k]);
    }
    return out;
  }
  return v;
}

function canonicalStringify(v){
  return JSON.stringify(canonicalize(v));
}

function enforceStaging(staging){
  if(!fs.existsSync(staging)){
    throw new Error('STAGING_DIR_NOT_FOUND');
  }

  const required = new Set([
    'sarif.json',
    'normalized.json',
    'diff.json',
    'baseline_used.json',
    'core_run_meta.json'
  ]);

  const optional = new Set([
    'report.md',
    'fix_plan.json',
    'apply_log.txt',
    'results.sarif'
  ]);

  const entries = fs.readdirSync(staging);
  const found = new Set(entries);

  for(const r of required){
    if(!found.has(r)){
      throw new Error('STAGING_MISSING_REQUIRED:'+r);
    }
  }

  for(const f of found){
    if(!required.has(f) && !optional.has(f)){
      throw new Error('STAGING_UNEXPECTED_FILE:'+f);
    }
  }
}

function enforceCrossConsistency(staging){
  const sarifPath = path.join(staging, 'sarif.json');
  const normPath = path.join(staging, 'normalized.json');
  const diffPath = path.join(staging, 'diff.json');
  const basePath = path.join(staging, 'baseline_used.json');

  const stagedNormalized = readJson(normPath);
  const stagedDiff = readJson(diffPath);

  const baselineRaw = readJson(basePath);
  const baseline = Array.isArray(baselineRaw) ? baselineRaw : [];

  const computedNormalized = normalizeSarif(sarifPath);
  const computedDiff = diffIssues(computedNormalized, baseline);

  const sNorm = canonicalStringify(stagedNormalized);
  const cNorm = canonicalStringify(computedNormalized);
  if(sNorm !== cNorm){
    throw new Error('STAGING_DERIVATION_MISMATCH:normalized');
  }

  const sDiff = canonicalStringify(stagedDiff);
  const cDiff = canonicalStringify(computedDiff);
  if(sDiff !== cDiff){
    throw new Error('STAGING_DERIVATION_MISMATCH:diff');
  }
}

export async function run(args){

  // INTERNAL_GOVERNANCE_BRANCH_GUARD
  if(process.env.NONO_ENFORCE_BRANCH === '1'){
    try{
      const cp = require('child_process');
      const branch = cp.execSync('git rev-parse --abbrev-ref HEAD').toString().trim();
      if(!branch.startsWith('release_') && !branch.startsWith('dev_')){
        throw new Error('BRANCH_POLICY_VIOLATION:'+branch);
      }
    }catch(e){
      throw new Error('BRANCH_POLICY_ENFORCEMENT_FAILED');
    }
  }


  const staging = args.staging ? String(args.staging) : null;

  if(staging){
    enforceStaging(staging);
    enforceCrossConsistency(staging);
  }

  let sarifInput = args.sarif;
  if(staging){
    sarifInput = path.join(staging, 'sarif.json');
  }

  const current = staging
    ? readJson(path.join(staging, 'normalized.json'))
    : normalizeSarif(sarifInput);

  const baseline = staging
    ? (Array.isArray(readJson(path.join(staging, 'baseline_used.json'))) ? readJson(path.join(staging, 'baseline_used.json')) : [])
    : (args.baseline && fs.existsSync(args.baseline)
        ? (Array.isArray(readJson(args.baseline)) ? readJson(args.baseline) : [])
        : []);

  const diff = staging
    ? readJson(path.join(staging, 'diff.json'))
    : diffIssues(current, baseline);

  const coreMeta = staging ? readJson(path.join(staging, 'core_run_meta.json')) : null;

  const policy = loadPolicy(args.policy);
  const decision = decide(diff, policy);

decision.engine_version = 'gate-1.1.0';

  const contractPath = path.join('contracts','contract.json');
  ensureContract(contractPath);
  const contractHash = hashContract(contractPath);
  decision.contract_sha256 = contractHash;

  const archPath = path.join(args.staging, 'ARCHITECTURE_STRICT_MODEL.md');
  const archShaPath = path.join(args.staging, 'ARCHITECTURE_SHA256.txt');
  if(fs.existsSync(archPath)){
    const archText = readText(archPath);
    const archActual = sha256Text(archText);
    if(fs.existsSync(archShaPath)){
      const archExpected = readText(archShaPath).trim();
      if(archExpected && archExpected !== archActual){
        throw new Error('ARCHITECTURE_SHA_MISMATCH');
      }
      decision.architecture_sha256 = archExpected || archActual;
    } else {
      fs.writeFileSync(archShaPath, archActual + '\n');
      decision.architecture_sha256 = archActual;
    }
  }

  const runId = args.runId ? String(args.runId) : new Date().toISOString().replace(/[:.]/g,'-');
  const outDir = path.join('.nono','evidence', runId);

  const report =
`# Nono Gate Report

Status: ${decision.status}

New Issues: ${decision.new_count}

Severity:
${JSON.stringify(decision.severity||{},null,2)}

Stale Waivers: ${(decision.stale_waivers||[]).length}

Contract SHA256: ${decision.contract_sha256 || ''}

Architecture SHA256: ${decision.architecture_sha256 || ''}

Staging: ${staging || ''}
`;

  decision.bundle_fingerprint = sha256Text(canonicalStringify(decision));

  const rootAnchor = sha256Text(
    (decision.bundle_fingerprint || '') + '|' +
    (decision.contract_sha256 || '') + '|' +
    (decision.architecture_sha256 || '')
  ) + '\n';

  const contractRaw = fs.readFileSync(contractPath,'utf-8');

  
  // PROVENANCE_ATTESTATION_V1
  function safeExec(cmd){
    try{
      const cp = require('child_process');
      return cp.execSync(cmd,{stdio:['ignore','pipe','ignore']}).toString().trim();
    }catch{
      return '';
    }
  }
  const provenance = canonicalize({
    schema: 'nono-gate-provenance-v1',
    generated_at: (process.env.NONO_DETERMINISTIC_RUNID === '1' ? ('det:'+String(decision.bundle_fingerprint||'')).slice(0,40) : new Date().toISOString()), // DETERMINISTIC_PROVENANCE_TIME_V1
    repo: safeExec('git config --get remote.origin.url'),
    commit: safeExec('git rev-parse HEAD'),
    branch: safeExec('git rev-parse --abbrev-ref HEAD'),
    engine: {
      name: 'nono-gate',
      version: decision.engine_version || ''
    },
    inputs: {
      policy_path: args.policy ? String(args.policy) : '',
      policy_sha256: (args.policy && fs.existsSync(args.policy)) ? crypto.createHash('sha256').update(fs.readFileSync(args.policy)).digest('hex') : '',
      contract_sha256: decision.contract_sha256 || '',
      architecture_sha256: decision.architecture_sha256 || '',
      bundle_fingerprint: decision.bundle_fingerprint || ''
    },
    runtime: {
      node: process.version,
      platform: process.platform,
      arch: process.arch
    }
  });
  const provenanceText = canonicalStringify(provenance) + '\n';
  const provenanceDet = canonicalize(Object.assign({}, provenance));
  delete provenanceDet.generated_at;
  const provenanceTextDet = canonicalStringify(provenanceDet) + '\n';
  const provenanceSha = sha256Text(provenanceTextDet);
  decision.provenance_sha256 = provenanceSha;
  // REPLAY_BIND_V1 (deterministic lineage replay proof, no timestamps)
  let lineageRootForEvidence = '';
  let lineageReplayReceiptText = '';
  try{
    const lineage = appendLineageEntry(process.cwd(), {
      run_id: runId,
      bundle_root_sha256: String(rootAnchor||'').trim(),
      bundle_fingerprint_sha256: String(decision.bundle_fingerprint||''),
      contract_sha256: String(decision.contract_sha256||''),
      architecture_sha256: String(decision.architecture_sha256||'')
    });

    decision.lineage_root_sha256 = String(lineage.lineage_root_sha256||'');
    decision.merkle_lineage_root_sha256 = String(lineage.merkle_root_sha256||'');
    decision.lineage_entry_sha256 = String(lineage.entry_sha256||'');
    lineageRootForEvidence = (decision.lineage_root_sha256||'') ? (String(decision.lineage_root_sha256) + '\n') : '';
    const merkleLineageRootForEvidence = (decision.merkle_lineage_root_sha256||'') ? (String(decision.merkle_lineage_root_sha256) + '\n') : '';
    // immutable replay verifier (recompute chain from LINEAGE.ndjson and compare with HEAD)
    const lineageDir = path.join(process.cwd(), '.nono', 'lineage');
    const logPath = path.join(lineageDir, 'LINEAGE.ndjson');
    const headPath = path.join(lineageDir, 'HEAD.json');

    const headObj = fs.existsSync(headPath) ? readJson(headPath) : {};
    const lines = fs.existsSync(logPath) ? readText(logPath).split(/\r?\n/).filter(Boolean) : [];

    let prev = '';
    let lastEntrySha = '';
    for(let i=0;i<lines.length;i++){
      const obj = JSON.parse(lines[i]);
      const entry = (obj && obj.entry) ? obj.entry : obj;

      const prevIn = String(entry.prev_root_sha256||'');
      const expectedPrev = (i === 0) ? '' : prev;
      if(prevIn !== expectedPrev){
        throw new Error('LINEAGE_PREV_MISMATCH_AT:'+i+':expected='+expectedPrev+':got='+prevIn);
      }

      const eText = canonicalStringify(entry) + '\n';
      const eSha = sha256Text(eText);
      const newRoot = sha256Text(expectedPrev + '|' + eSha);

      prev = newRoot;
      lastEntrySha = eSha;
    }

    const expectedHead = (headObj && headObj.lineage_root_sha256) ? String(headObj.lineage_root_sha256) : '';
    if(expectedHead && expectedHead !== prev){
      throw new Error('LINEAGE_HEAD_MISMATCH:expected='+expectedHead+':computed='+prev);
    }

    const receiptDet = canonicalize({
      schema_id: 'nono.lineage.replay.receipt.v1',
      ok: true,
      entries: lines.length,
      computed_head_root_sha256: prev,
      computed_last_entry_sha256: lastEntrySha,
      computed_merkle_root_sha256: (headObj && headObj.merkle_root_sha256) ? String(headObj.merkle_root_sha256) : '',
      head_expected_root_sha256: expectedHead
    });

    lineageReplayReceiptText = canonicalStringify(receiptDet) + '\n';
    decision.lineage_replay_ok = true;
    decision.lineage_chain_height = lines.length;
  }catch(e){
    decision.lineage_replay_ok = false;
    decision.lineage_chain_height = 0;
    decision.lineage_replay_error = String(e && e.message ? e.message : 'LINEAGE_REPLAY_FAILED');

    const receiptDet = canonicalize({
      schema_id: 'nono.lineage.replay.receipt.v1',
      ok: false,
      error: String(decision.lineage_replay_error||'LINEAGE_REPLAY_FAILED')
    });

    lineageReplayReceiptText = canonicalStringify(receiptDet) + '\n';
  }

  const evidenceFiles = {
    'baseline_used.json': baseline,
    'normalized.json': current,
    'diff.json': diff,
    'core_run_meta.json': coreMeta === null ? {} : coreMeta,
    'decision.json': decision,
    'report.md': report,
    'contract.json': contractRaw,
    'PROVENANCE.json': provenanceText,
    'PROVENANCE_DET.json': provenanceTextDet,
    'LINEAGE_ROOT_SHA256.txt': lineageRootForEvidence,
    'MERKLE_LINEAGE_ROOT_SHA256.txt': (typeof merkleLineageRootForEvidence !== 'undefined' ? merkleLineageRootForEvidence : ''),
    'LINEAGE_REPLAY_RECEIPT.json': lineageReplayReceiptText,
    'BUNDLE_ROOT_SHA256.txt': rootAnchor
  };

  writeEvidence(outDir, evidenceFiles);
  // ---- DETERMINISTIC MANIFEST (non-circular) ----
  const files = fs.readdirSync(outDir)
    .filter(f => f !== 'SHA256SUMS.txt')
    .sort();

  const lines = [];

  for (const f of files) {
    const full = path.join(outDir, f);
    const buf = fs.readFileSync(full);
    const hash = crypto.createHash('sha256').update(buf).digest('hex');
    lines.push(hash + '  ' + f);
  }

  fs.writeFileSync(
    path.join(outDir, 'SHA256SUMS.txt'),
    lines.join('\n') + '\n',
    { encoding: 'utf8' }
  );


  await finalizeWithLedger({ decision, outDir }, runId);
  if(decision && decision.merkle_lineage_root_sha256){
    const merkleTxt = String(decision.merkle_lineage_root_sha256) + '\n';
    fs.writeFileSync(path.join(outDir,'MERKLE_LINEAGE_ROOT_SHA256.txt'), merkleTxt, { encoding:'utf8' });
  }

  
  console.log('EVIDENCE_DIR='+outDir);
  return { decision, outDir };
}

















